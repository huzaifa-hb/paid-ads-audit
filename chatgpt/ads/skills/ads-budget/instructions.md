---
name: ads-budget
description: "Budget allocation and bidding review across ad platforms. Evaluates spend distribution, strategy fit, scaling readiness, and evidence-backed pause or scale candidates."
---
...
1. Follow `references/data-sources.md` and collect aligned budget and performance data
   from every active platform, plus commerce/lifecycle sources when relevant.
...
8. Validate that pause or scale candidates have enough data for the account's economics,
   conversion lag, and variance; fixed click/spend thresholds are only heuristics.
9. Generate evidence-backed pause-review and scale-candidate lists.
...
|----------|---------------|--------|
| CPA >3x target | Enough data after conversion lag | Review for pause; verify tracking, marginal value, and strategic role first |
...
- Pause-review list: campaigns/ad groups requiring an explicit decision
