...
Assess whether the account supplies genuinely distinct concepts, angles, and formats
rather than cosmetic variants. Treat any named delivery-system thresholds in the
references as hypotheses to verify against current Meta documentation and observed
account behavior, not as universally exposed account metrics.

## Process

1. Follow `references/data-sources.md` and query the connected Meta Ads source first;
   use exports or Events Manager screenshots only for unavailable fields.
2. Read `references/meta-audit.md` for full 50-check audit
