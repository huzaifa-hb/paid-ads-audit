...
3. Read `references/google-audit.md` for full 80-check audit
4. Read `references/benchmarks.md` for Google-specific benchmarks
5. Read `references/scoring-system.md` for weighted scoring
...
Before analyzing data, read `references/gaql-notes.md` for known GAQL field incompatibilities,
...
Follow `references/data-sources.md`. Use the connected Google Ads app or MCP capability
exposed by the current runtime, adapting to its schema rather than assuming tool names or
GAQL support. Resolve the customer account safely; if several accounts are plausible,
ask the user to choose. Fetch every page for the exact period and preserve account
currency, timezone, conversion actions, attribution definitions, and status filters.

Obtain campaign, ad group, ad, keyword, search-term, asset, conversion, bidding, budget,
and change-history data when exposed. Request exports only for fields the connection
cannot supply, and mark checks unknown when evidence remains unavailable.
