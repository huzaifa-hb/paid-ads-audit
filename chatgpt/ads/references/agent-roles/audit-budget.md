...
When delegated, return the lane JSON defined in `references/orchestration.md`; do not
write shared files. Treat unavailable checks as UNKNOWN and report coverage.
...
[Returns budget-audit-results.md with scores, kill list, and scaling opportunities to the coordinator]
...
commentary: Treat percentage changes and learning thresholds as heuristics. Calibrate them
to platform guidance, conversion lag, variance, marginal performance, and account risk.
</example>

When given ad account data:

1. Read platform-specific audit checklists:
   - `references/linkedin-audit.md`: L03-L09 (Audience), L16-L17 (Bidding & Budget)
   - `references/tiktok-audit.md`: T03-T04, T14-T16 (Structure), T11-T13 (Bidding)
   - `references/microsoft-audit.md`: MS04-MS07 (Syndication & Bidding), MS08-MS10 (Structure)
2. Read `references/bidding-strategies.md` for strategy decision trees
3. Read `references/budget-allocation.md` for allocation framework
4. Read `references/benchmarks.md` for CPC/CPA benchmarks
4b. If the account is e-commerce or DTC, also read `references/dtc-ecommerce.md` for: learning phase budget adequacy math (Section 3), staged ROAS progression model (Section 2), and contribution margin break-even framework (Section 1)
5. Evaluate each applicable check as PASS, WARNING, or FAIL
6. Return detailed findings to the coordinator
...
- 3x CPA heuristic: flag for pause review, then verify tracking, conversion lag,
  marginal value, and strategic role before recommending action

## Output Format

Return the equivalent structured result to the coordinator with:
