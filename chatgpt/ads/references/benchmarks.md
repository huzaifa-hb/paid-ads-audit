...
<!-- Compiled reference: verify volatile claims against current primary platform sources. -->
...
## DTC E-commerce Funnel Benchmarks

Use these to identify conversion leaks and prioritize fixes. Prioritize by volume × benchmark gap, not relative drop rate alone. See `dtc-ecommerce.md` Section 7 for the full prioritization formula.

### Session-to-Purchase Funnel (DTC Shopify)

| Funnel Stage | Rate | DTC Home Goods | DTC Apparel | DTC Beauty | Status if Below |
|--------------|------|---------------|-------------|------------|-----------------|
| Sessions → PDP view | 40–60% | 45–55% | 40–55% | 50–65% | Below 35% = traffic quality or navigation issue |
| PDP view → Add to Cart | 8–15% | 10–15% | 8–14% | 10–18% | Below 7% = PDP friction (see dtc-ecommerce.md Section 8) |
| Add to Cart → Checkout initiated | 65–80% | 70–80% | 65–78% | 70–82% | Below 55% = cart page friction or shipping cost shock |
| Checkout initiated → Purchase | 60–75% | 65–75% | 60–72% | 65–78% | Below 50% = checkout UX, payment options, or form friction |
| **Store-wide CVR** | **1.5–3.5%** | **1.5–2.5%** | **1.2–2.0%** | **2.0–4.0%** | Below 1% = multiple compounding issues |

### Mobile vs Desktop CVR Split (DTC Shopify)

| Device | Typical CVR Range | Healthy Gap | Flag |
|--------|------------------|-------------|------|
| Desktop | 2.0–4.0% | — | — |
| Mobile | 1.0–2.5% | Mobile is 40–60% of desktop CVR | If mobile < 50% of desktop CVR → mobile friction is costing significant revenue |

Mobile is typically 70–85% of DTC traffic. A large mobile/desktop CVR gap is usually the highest-leverage CRO opportunity on the account. See `dtc-ecommerce.md` Section 8 for ordered mobile fix sequence.

### How to Prioritize Funnel Fixes

```
Priority Score = Visitors lost at stage × (Benchmark midpoint − Current rate)

Fix stages in descending Priority Score order, not by which rate looks worst.
Top-of-funnel fixes have higher leverage because volume is larger.
```
