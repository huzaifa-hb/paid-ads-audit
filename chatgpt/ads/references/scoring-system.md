...
<!-- Compiled reference: verify volatile thresholds against current platform documentation. -->
...
| FAIL | 0 points |
| UNKNOWN / NOT ASSESSED | Excluded from total possible; reported in coverage |
| N/A | Excluded from total possible |

Always show `evaluated applicable checks / applicable checks` and coverage percentage
beside the score. Do not treat missing connector data as PASS or FAIL. If coverage is too
low to support a useful score, return no score and explain the missing evidence.
...
Cross-platform checks produce findings and do not add a second scoring layer to the
spend-weighted platform aggregate.
...
When auditing multiple platforms, calculate per-platform scores then aggregate only if
verified spend covers every included platform for the same period and currency:
